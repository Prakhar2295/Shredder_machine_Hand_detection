...
def drawboxtosafeline(image_np,p1,p2,Line_Position2,Orientation):
    ...
    if(Orientation=="bt"):
        ...
        if (bounding_mid):
            cv2.line(img = image_np,pt1 = bounding_mid, pt2 =(bounding_mid[0],Line_Position2),color = (0,0,255),thickness = 1,lineType = 8,shift = 0)
            distance_from_line = bounding_mid[1] - Line_Position2
        elif (Orientation=="tb"):
            bounding_mid = (int((p1[0]+ p2[0])/2),int(p2[1]))
            if (bounding_mid):
                cv2.line(img = image_np,pt1 = bounding_mid, pt2 = (Line_Position2,bounding_mid[1]),color = (0,0,255),thickness = 1,lineType = 8,shift = 0)
        ...
        if (distance_from_line <= 0):
            ...
            cv2.putText(image_np, "ALERT", (posii,50),cv2.FONT_HERSHEY_COMPLEX,0.75,(255,0,0),2)
            playsound(r"C:\Users\prath\Desktop\PYTHON\Full_stack_Data_Science\Architectures\Hand-Detection-and-Distance-Safety\utils\alert.wav")
